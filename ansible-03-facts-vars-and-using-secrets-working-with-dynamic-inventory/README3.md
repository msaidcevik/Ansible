...
```
```bash
"setup" module node1 in bütün bilgilerini getirir.
```
```
...
```
```
var: ansible_facts  var=variable ansible_facts bütün metadata yıo getirir.
```
- run the play book
...
```
```
> işareti multiline çoklu satır için kullanılıyor.
```
- run the playbook
