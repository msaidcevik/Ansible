...
We use ssh and sshd for secure encrypted communications between two untrusted hosts over 
an insecure network or internet
OpenSSH SSH daemon Linux de sshd olarak adlandırılır ve ssh komutu tarafından kullanılır.
Güvenli olmayan bir network üzerinden güvenilmeyen iki ana bilgisayar arasında güvenli 
şifreli iletişim için ssh ve sshd kullanılır.
```
...
"-h" --human-readable
disk şeklindeki file lerin kullanım yüzdesini gösterir.
```
...
```
```bash
ilk komutun çalışıp çalışmadığından emin olmak için 
