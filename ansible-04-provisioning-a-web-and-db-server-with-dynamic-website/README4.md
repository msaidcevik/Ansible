...
start mariadb kısmını yazmadan systemd module de "state: started" yaparsak aynı işlemi alırız.
...
ilk önce started edecek sonra enabled edecek.
```
...
```bash
"MAriaDB" nin içine girmeden shell command ile istenileni alabiliriz
...
```
```
priv database e bütün yetkileri verdi.
```
